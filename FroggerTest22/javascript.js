...
//     time = Date.now();
//     elapse = time - timeInMs;
// if (elapse>10000){
//     snakeArray.push(createImage("resources/snake.png", "CarBOTTOM", 100, 250, 30, 30));
// }
    if (waterCollision == true && frog.top < 250 && frog.top>50) {
        frog.left = 325;
        frog.top = 600;
...
function drawCars() {
    ...
    elapse = time - timeInMs;
    if (elapse>10000){
        snakeArray.push(createImage("resources/snake.png", "CarBOTTOM", 100, 250, 30, 30));//ASK mr.CORTEZ WHY DRAWS LIKE THIS?
    }
    for(i = 0;i<pictArray.length;i++){
        if (level==1) {
            pictArray[i].left += .5;
    ...
    for(m=0;m<snakeArray.length;m++){
        if (level==1){
            snakeArray[m].left += 1;
            ctx.drawImage(snakeArray[m], snakeArray[m].left, snakeArray[m].top, 55, 50);
        }
    }
