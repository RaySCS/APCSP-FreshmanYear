...
def displayRules():
    askName = input("Hello, What's your first name?")
...
def scenarios(askLevel,compChoice,Attempts,games):
  ...
  global gamesWon
  global gamesLost
  positionCheck1 = False#In order to detect when black peg is neccessary to be added. Purpose of all 4.
  positionCheck2 = False#Determine if a black peg is potentially there, that is if white peg is there too
  positionCheck3 = False
  positionCheck4 = False
  playAgain = False
  BlackPegs = 0
  ...
  for i in range(0, len(compChoice)):
      white = False#This nested for loop is similar to the hangman idea, going through each color and checking user guess share anything with computer selected colors
      for b in range(0, len(userGuess)):
          if (compChoice[i] == userGuess[b]):
              white = True  # If any match found, boolean is true, white peg is true then
              WhitePegs += 1# A white peg is added after nested for loop.
  if compChoice[0] == userGuess[0]:
      positionCheck1 = True#Bascially, going through the index and checking every position for a position match
  if compChoice[1] == userGuess[1]:
      positionCheck2 = True#Signyfing that a black peg might be possible(throughut all four)
  if compChoice[2] == userGuess[2]:
      positionCheck3 = True
  if compChoice[3] == userGuess[3]:
      positionCheck4 = True
  if white == True and positionCheck1 == True:
      BlackPegs += 1#If the white peg is true and there is a position match(possible 4 outcomes below) then there is a black peg
  if white == True and positionCheck2 == True:
      BlackPegs += 1
  if white == True and positionCheck3 == True:
      BlackPegs += 1
  ...
  print("Black Pegs: ", BlackPegs)
  print("White Pegs: ", newWhitePeg)
  print("Mr.Cortez purposes answer is: ", compChoice)
  if playAgain==True:
...
#What I did for A: 2 player, 3 categories, multiple times playable, duplicates, personaliaztion
displayRules()
