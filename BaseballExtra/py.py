...
def preMLB():
    ...
    innings = 9#Simulating a college world series and depending on whoever wins the single match, like a game 7, makes the MLB
...
def playBaseball(team1, team2, names, askTeamName, numPlayer):
    ...
    maxHomeRun = []
    maxHits = []
    BobAge = random.randint(20, 30)
    JoeAge = random.randint(20, 30)
    SteveAge = random.randint(20, 30)
    MikeAge = random.randint(20, 30)
    LukeAge = random.randint(20, 30)
    MarkAge = random.randint(20, 30)
    JohnAge = random.randint(20, 30)
    AaronAge = random.randint(20, 30)
    JamesAge = random.randint(20, 30)
    #Below is the other team
    JoelAge = random.randint(20, 30)
    RichardAge = random.randint(20, 30)
    FrankAge = random.randint(20, 30)
    LukasAge = random.randint(20, 30)
    JacksonAge = random.randint(20, 30)
    StephenAge = random.randint(20, 30)
    AbrahamAge = random.randint(20, 30)
    ChrisAge = random.randint(20, 30)
    IsiahAge = random.randint(20, 30)
    ...
    if numPlayer == 1:
        ...
        if BobAge>=35:
            a = 'Bob'
            names.remove(a)#Player retires if there age is greater than 35 or equal to it.
            newPlayer = "Kobe"
            names.append(newPlayer)#Add a new player when that persons time has come to retire
        if JoeAge>=35:
            a = 'Joe'
            names.remove(a)#Player retires
            newPlayer = "Jake"
            names.append(newPlayer)#Add a new player when that persons time has come to retire
        if SteveAge>=35:
            a = 'Steve'
