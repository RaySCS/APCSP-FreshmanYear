...
def baseBallSelectionPlayers():
  ...
  playBaseball(team1, team2, names)


def playBaseball(team1, team2, names):
                ...
                while outs != 3:
                    ...
                    if determineHit <= team1.players[randomPlayer].average:
                        ...
                        if team1.players[randomPlayer].average > 10 and team1.players[randomPlayer].average <50 and nothingElse==False:
                            ...
                            hitsHome += 1#I think I may have been too leniant on how good players are, this the reason for the high scores.(0-20)
                            hitsTotalHome+=1
                            nothingElse = False
    ...
    team2.show_StatsA()
    print("The Home Team has won: ", homeTeamWins, "games")
    ...
    for j in range(0,8):
        maxDouble.append(team1.players[x].double)
        maxDouble.append(team2.players[x].doubleA)
    ...
    for o in range(0,8):
        maxTriple.append(team1.players[x].triple)
        maxTriple.append(team2.players[x].tripleA)
        player2 = random.randint(0,8)
        hitter2 = names[player2]
        for z in maxTriple:
            if z>maxT:
                maxT = z
    print("The Best in the Category of Triples is: ", hitter2, "with", maxT, "triples")
    for e in range(0,8):
        maxHomeRun.append(team1.players[x].homerun)
        maxHomeRun.append(team2.players[x].homerunA)
        player3 = random.randint(0,8)
        hitter3 = names[player3]
        for a in maxHomeRun:
            if a >maxH:
                maxH = a
    print("The Best in the Category of Home Runs is: ", hitter3, "with", maxH, "home runs")
    for q in range(0,8):
        maxHits.append(team1.players[x].HTS)
        maxHits.append(team2.players[x].HTSA)
        player4 = random.randint(0,8)
        hitter4 = names[player4]
        for b in maxHits:
            if b>maxHT:
                maxHT = b
    print("The Best in the Category of Hits is: ", hitter4, "with", maxHT, "hits")
