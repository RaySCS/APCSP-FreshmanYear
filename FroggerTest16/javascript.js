...
function drawCars() {
    ...
    for(x = 0;x<array3.length;x++){
        if (level==1){
            array3[x].left += .5;
            ctx.drawImage(array3[x], array3[x].left, array3[x].top, 55, 55);
        }
        if (level==2) {
            array3[x].left += 1;
            ctx.drawImage(array3[r], array3[x].left, array3[x].top, 55, 55);
        }
        if(array3[x].left>650){
            array3[x].left = -200;
        }
    }
