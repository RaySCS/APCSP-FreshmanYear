...
def displayRules():
    ...
    print("Rules: ")
    print(
        "-The computer selects a sequence of four colors out of six, which are red, green, orange, blue, yellow, and purple")
...
def beginGame():
    ...
    if askGameMode=="one":
        onePlayer(Attempts, games, i)
    if askGameMode=="two":
        twoPlayer()#LEFT OFF WORKING ON PLAYER TWO, SEPERATE BY COMMA
def twoPlayer():
    ...
    askPlayerOne = input("Player one, input the sequence of four things you wish to code(commas), remember sequence matters")
    playerOneEntry = askPlayerOne.split(",") # Allows for Letters to be splitted. Gives in an array format. Could split by any character
    print(playerOneEntry)
def onePlayer(Attempts, games, i,):
