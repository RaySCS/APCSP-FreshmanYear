...
function generateCharacter() {
    ...
    for(a = 0;a<=2;a++){
        ...
        genNumI = Math.floor(Math.random()*(4));//In order to pick a random career interest
...
function displayRoll(roll, askUserResponse) {
    ...
    document.getElementById("outputCash").innerHTML = "Cash: " + cash;
    askChoices(roll, askUserResponse)
}


function askChoices(roll, askUserResponse) {
    //The better the roll, then the more money you can make and get further ahead.
    if (roll==3){
        cash +=150;//different scenarios which can occur. Some randomness can be involved. Statistics, you can incorporate.
        document.getElementById("outputCash").innerHTML = "Cash: " + cash;
    }
    if (roll==2){
        cash +=100;//different scenarios which can occur. Some randomness can be involved. Statistics, you can incorporate.
        document.getElementById("outputCash").innerHTML = "Cash: " + cash;
    }
    if (roll==1){
        cash +=50;//different scenarios which can occur. Some randomness can be involved. Statistics, you can incorporate.
        document.getElementById("outputCash").innerHTML = "Cash: " + cash;
    }
    var firstChoice = false;
    for (a=0;a<Richard.length;a++){
        if (Richard[a] == "Computer Science"){
            document.getElementById("outputQuestion").innerHTML = "CS"//Need to do it for each character. EXPAND UPON THIS!
        }
        if (Richard[a] == "Engineering"){
            document.getElementById("outputQuestion").innerHTML = "E"//Need to do it for each character. EXPAND UPON THIS!
        }
        if (Richard[a] == "Chemistry"){
            document.getElementById("outputQuestion").innerHTML = "C"//Need to do it for each character. EXPAND UPON THIS!
        }
        if (Richard[a] == "Law"){
            document.getElementById("outputQuestion").innerHTML = "L"//Need to do it for each character. EXPAND UPON THIS!
        }
    }
    for (n=0;n<Nicole.length;n++){
        ...
        if (Nicole[n] == "Engineering"){
            document.getElementById("outputQuestion").innerHTML = "E"//Need to do it for each character. EXPAND UPON THIS!
        }
        if (Nicole[n] == "Chemistry"){
            document.getElementById("outputQuestion").innerHTML = "C"//Need to do it for each character. EXPAND UPON THIS!
        }
        if (Nicole[n] == "Law"){
            document.getElementById("outputQuestion").innerHTML = "L"//Need to do it for each character. EXPAND UPON THIS!
