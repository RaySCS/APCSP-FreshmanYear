...
#Each persons hits and types of hits should be kept track of
def playBall():
...
def baseBallSelectionPlayers():
  ...
  runMultiple(team1, team2, askTeamName)

def runMultiple(team1, team2, askTeamName):
    for x in range(5):
        ...
        homeTeamWins = 0
        awayTeamWins = 0
        playBaseball(team1, team2, askTeamName, homeScore, awayScore, possession, innings, hitsTotalHome, hitsTotalAway,counter,homeTeamWins, awayTeamWins)#Abstracting and running multiple games here

def playBaseball(team1, team2, askTeamName, homeScore, awayScore, possession, innings, hitsTotalHome, hitsTotalAway,counter,homeTeamWins, awayTeamWins):
    for x in range(1, innings): # loops 9 times for the 9 innings. IF tie occurs, then innings is added
            ...
            while outs != 3:
                ...
                if determineHit <= team1.players[randomPlayer].average:
                    ...
                    if team1.players[randomPlayer].average > 10 and team1.players[randomPlayer].average <50 and nothingElse==False:
                        ...
                        hitsHome += 1#I think I may have been too leniant on how good players are, this the reason for the high scores.
                        hitsTotalHome+=1
                        nothingElse = False
                    if team1.players[randomPlayer].speed > 80 and team1.players[randomPlayer].speed < 40 and nothingElse==False:
        ...
        if possession == 2:
            ...
            if runsHome == runsAway:
                innings += 1#IF there is a tie, then simply add one more run
        if runsAway > runsHome:
            awayTeamWins += 1
        if runsHome > runsAway:
            homeTeamWins += 1
        homeScore.append(runsHome)#Adding the amount of runs in that inning
    ...
    print(askTeamName,":" + home_print1 + ": " + str(sum(homeScore)))
    print("Away :" + away_print2 + ": " + str(sum(awayScore)))
    print(askTeamName," Team Total Hits:", hitsTotalHome,"H")#H represents the teams total hits
    ...
    team1.show_Stats()
    print("Stats of the opponents team")
    print("Name    S   D   T   H   Hits")
    print("The", askTeamName, "Team has won: ", homeTeamWins)
    print("The Away Team has won: ", awayTeamWins)
    team2.show_StatsA()
    if counter == 9:
        ...
        if ask=="yes":
            ...
            playBaseball(team1, team2, askTeamName, homeScore, awayScore, possession, innings, hitsTotalHome, hitsTotalAway,counter,homeTeamWins, awayTeamWins)
        if ask == "no":
